...
namespace shyft {
    namespace core {
        /** \brief pt_us_k namespace declares the needed pt_us_k specific parts of a cell
        * that is:
        * -# the pt_us_k parameter, state and response types
        ...
        */
            ...
            */
            struct all_response_collector {
                ...
                void collect(size_t idx, const response_t& response) {
                    ...
                    snow_sca.set(idx, response.us.sca);
                    snow_outflow.set(idx, response.us.outflow); //TODO: current mm/h.. but  want m3/s, but we get mm/h from snow output
                    glacier_melt.set(idx, response.gm_melt_m3s);
                    snow_swe.set(idx, response.us.storage);
            ...
            struct state_collector {
                ...
                pts_t us_albedo;
                pts_t us_lwc;
                pts_t us_surface_heat;
                pts_t us_alpha;
                pts_t us_sdc_melt_mean;
                pts_t us_acc_melt;
                pts_t us_iso_pot_energy;
                pts_t us_temp_swe;
                ...
                state_collector(const timeaxis_t& time_axis)
                 : collect_state(false), destination_area(0.0),
                   ...
                   us_albedo(time_axis, 0.0),
                   us_lwc(time_axis, 0.0),
                   us_surface_heat(time_axis, 0.0),
                   us_alpha(time_axis, 0.0),
                   us_sdc_melt_mean(time_axis, 0.0),
                   us_acc_melt(time_axis, 0.0),
                   us_iso_pot_energy(time_axis, 0.0),
                   us_temp_swe(time_axis, 0.0)
                   { /* Do nothing */ }
                ...
                void initialize(const timeaxis_t& time_axis,int start_step,int n_steps, double area) {
                    ...
                    ts_init(us_albedo, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_lwc, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_surface_heat, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_alpha, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_sdc_melt_mean, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_acc_melt, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_iso_pot_energy, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                    ts_init(us_temp_swe, ta, start_step, n_steps, ts_point_fx::POINT_INSTANT_VALUE);
                }
                ...
                void collect(size_t idx, const state_t& state) {
                    if (collect_state) {
                        ...
                        us_albedo.set(idx, state.us.albedo);
                        us_lwc.set(idx, state.us.lwc);
                        us_surface_heat.set(idx, state.us.surface_heat);
                        us_alpha.set(idx, state.us.alpha);
                        us_sdc_melt_mean.set(idx, state.us.sdc_melt_mean);
                        us_acc_melt.set(idx, state.us.acc_melt);
                        us_iso_pot_energy.set(idx, state.us.iso_pot_energy);
                        us_temp_swe.set(idx, state.us.temp_swe);
                    }
                }
            };
        ...
        template<>
        inline void cell<pt_us_k::parameter_t, environment_t, pt_us_k::state_t,
                         pt_us_k::state_collector, pt_us_k::all_response_collector>
            ::run(const timeaxis_t& time_axis, int start_step, int n_steps) {
            if (parameter.get() == nullptr)
                throw std::runtime_error("pt_us_k::run with null parameter attempted");
            begin_run(time_axis,start_step,n_steps);
            pt_us_k::run_pt_us_k<direct_accessor, pt_us_k::response_t>(
                geo,
                *parameter,
        ...
        template<>
        inline void cell<pt_us_k::parameter_t, environment_t, pt_us_k::state_t,
                         pt_us_k::null_collector, pt_us_k::discharge_collector>
            ::run(const timeaxis_t& time_axis, int start_step, int n_steps) {
            if (parameter.get() == nullptr)
                throw std::runtime_error("pt_us_k::run with null parameter attempted");
            begin_run(time_axis, start_step, n_steps);
