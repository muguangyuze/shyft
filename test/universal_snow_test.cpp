...
namespace us=shyft::core::universal_snow;
class universal_snow_test {
public:
    template <class US_CALC,class P>
    void reset_snow_pack(US_CALC&usc, double& sca, double& lwc, double& alpha, double& sdc_melt_mean, double& acc_melt,
        double& temp_swe, const double storage, const P& p) const {
        usc.reset_snow_pack(sca, lwc, alpha, sdc_melt_mean, acc_melt, temp_swe, storage, p);
    }
    template <class US_CALC>
    void calc_snow_state(US_CALC&usc, const double shape, const double scale, const double y0, const double lambda,
        const double lwd, const double max_water_frac, const double temp_swe,
        double& swe, double& sca) const {
        usc.calc_snow_state(shape, scale, y0, lambda, lwd, max_water_frac, temp_swe, swe, sca);
    }
    template <class US_CALC>
    double corr_lwc(US_CALC&usc, const double z1, const double a1, const double b1,
        double z2, const double a2, const double b2) const {
        return usc.corr_lwc(z1, a1, b1, z2, a2, b2);
    }
};
static universal_snow_test fix; // access to private scope
TEST_SUITE("universal_snow") {
    TEST_CASE("us_reset_snow_pack_zero_storage") {

        us::calculator<us::parameter, us::state, us::response> us;
        us::parameter p;
        double sca;
        double lwc;
        double alpha;
        ...
        fix.reset_snow_pack(us, sca, lwc, alpha, sdc_melt_mean, acc_melt, temp_swe, storage, p);
    ...
    TEST_CASE("us_reset_snow_pack_with_storage") {
    ...
    TEST_CASE("us_calculate_snow_state") {
        ...
        double swe; // Output from us.calc_snow_state(...)
        double sca; // Output from us.calc_snow_state(...)
        fix.calc_snow_state(us, shape, scale, y0, lambda, lwd, max_water_frac, temp_swe, swe, sca);
        TS_ASSERT_DELTA(swe, 0.384, shyfttest::EPS);
        TS_ASSERT_DELTA(sca, 0.96, shyfttest::EPS);
    }

    TEST_CASE("us_correct_lwc") {
        ...
        const double result = fix.corr_lwc(us, z1, a1, b1, z2, a2, b2);
        // as a result of using lower resolution, less accuracy
        TS_ASSERT_DELTA(result, 3.8411, 0.0001);
        // would throw in previous versions
        fix.corr_lwc(us, 1.0, 6.25, 0.005358, 0.5, 6.25, 0.005358);

        fix.corr_lwc(us, 0.0, a1, b1, z2, a2, b2);
        fix.corr_lwc(us, z1, a1, b1, 0.0, a2, b2);

    }

    TEST_CASE("us_warm_winter_effect") {
        us::calculator<us::parameter, us::state, us::response> us;
        double tx = -0.5;
        ...
        double srd = 5.0;
        double glac_alb = 0.4;
        us::parameter param(100, tx, wind_scale, wind_const, max_lwc, surf_mag, max_alb, min_alb, fadr, sadr, srd, glac_alb);
        ...
        double temp_swe = 0.0;
        us::state states(albedo, lwc, surface_heat, alpha, sdc_melt_mean, acc_melt, iso_pot_energy, temp_swe);
        ...
        for (size_t i = 0; i < 24 * num_days; ++i) {
            us.step(states, response, dt * 24 * 232 + i*dt, dt, param, temp, rad, prec, wind_speed, rel_hum, 0.0, 0.0);
    ...
    TEST_CASE("us_step") {
        ...
        us::state states(0.0, 1.0, 0.0, 1.0 / (param.snow_cv*param.snow_cv), 10.0, -1.0, 0.0, 0.0);
        us::response response;
        ...
        for (size_t i = 0; i < 365; ++i) {
            us.step(states, response, i*dt, dt, param, i & 1 ? temp : param.tx, rad, prec, wind_speed, rel_hum, 0.0, 0.0);
        }
    ...
    TEST_CASE("us_output_independent_of_timestep") {
        us::calculator<us::parameter, us::state, us::response> us;
        us::parameter param;
        auto dt = shyft::core::deltahours(1);
        auto dt3h = shyft::core::deltahours(3);

        us::state initial_state(0.0, 0.0, 0.0, 1.0 / (param.snow_cv*param.snow_cv), 0.0, -1.0, 0.0, 0.0);
        double rad = 10.0;
        double prec = 5.0;
        double wind_speed = 2.0;
        double rel_hum = 0.70;
        ...
        for (size_t j = 0;j < 2;j++) {
            ...
            // then with cold,
            us::state state1h(initial_state);
            us::response response1h;
            double output_hour = 0;
            for (size_t i = 0;i < 3;++i) {
                us.step(state1h, response1h, i*dt, dt, param, temp, rad, prec, wind_speed, rel_hum, 0.0, 0.0);
                output_hour += response1h.outflow; // mm_h
            }
            output_hour /= 3.0; // avg hour output
            us::state state3h(initial_state);
            us::response response3h;
            us.step(state3h, response3h, 0 * dt3h, dt3h, param, temp, rad, prec, wind_speed, rel_hum, 0.0, 0.0);
    ...
    TEST_CASE("us_forest_altitude_dependent_snow_cv") {
        us::parameter p;
