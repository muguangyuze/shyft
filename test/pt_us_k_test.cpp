...
TEST_CASE("us_call_stack") {
    ...
    us::state us_state(0.0, 1.0, 0.0, 1.0/(us_param.snow_cv*us_param.snow_cv), 10.0, -1.0, 0.0, 0.0);

    // Initialize response
    //response response;

    // Initialize collectors
    shyfttest::mock::PTUSKResponseCollector response_collector(time_axis.size());
    shyfttest::mock::StateCollector<ta::point_dt> state_collector(state_axis);

    state state{us_state, kirchner_state};
    parameter parameter{pt_param, us_param, ae_param, k_param,  p_corr_param};
    geo_cell_data geo_cell;
    pt_us_k::run_pt_us_k<shyft::time_series::direct_accessor,
...
TEST_CASE("us_raster_call_stack") {
    //using shyfttest::TSSource;

    typedef MCell<response, state, parameter, xpts_t> PTUSKCell;
    calendar cal;
    utctime t0 = cal.time(YMDhms(2014, 8, 1, 0, 0, 0));
    const int nhours=1;
    ...
    std::vector<PTUSKCell> model_cells;
    ...
    kr::state kirchner_state{5.0};
    us::state us_state(0.6, 1.0, 0.0, 1.0/(us_param.snow_cv*us_param.snow_cv), 10.0, -1.0, 0.0, 0.0);
    state state{us_state, kirchner_state};
    ...
    for (size_t i = 0; i < n_dests; ++i) {
        shyfttest::create_time_series(temp, prec, rel_hum, wind_speed, radiation, t0, dt, n_ts_points);
        state.us.albedo += 0.3*(double)i/(n_dests - 1); // Make the snow albedo differ at each destination.
        model_cells.emplace_back(temp, prec, wind_speed, rel_hum, radiation, state, parameter, i % 3);
    }


    const std::clock_t start = std::clock();
    for_each(model_cells.begin(), model_cells.end(), [&time_axis, &catchment_discharge,&state_axis] (PTUSKCell& d) mutable {
        ...
        //PTUSKResponseCollector rc(time_axis.size());

        pt_us_k::run_pt_us_k<shyft::time_series::direct_accessor, response>(d.geo_cell_info(), d.parameter(), time_axis,0,0,
              d.temperature(),
              d.precipitation(),
              d.wind_speed(),
              d.rel_hum(),
...
TEST_CASE("us_mass_balance") {
    ...
    us::state us_state;// zero snow (0.6, 1.0, 0.0, 1.0/(us_param.snow_cv*us_param.snow_cv), 10.0, -1.0, 0.0, 0.0);
    us_state.lwc=0.0;
    us_state.acc_melt=-1; // zero snow, precipitation goes straight through
    state state{us_state, kirchner_state};
    pt_us_k::state_collector sc;
    pt_us_k::all_response_collector rc;
    ...
    pt_us_k::run_pt_us_k<direct_accessor,pt_us_k::response>(gcd,parameter,tax,0,0,temp,prec,wind_speed,rel_hum,radiation,state,sc,rc);
    ...
    SUBCASE("direct_response_on_bare_lake_only") {
        ...
        state.us.lwc=0.0;// no snow, should have direct response, 3 mm/h, over the cell_area
        state.us.acc_melt=-1;// should be no snow, should go straight through
        pt_us_k::run_pt_us_k<direct_accessor,pt_us_k::response>(gcd,parameter,tax,0,0,temp,prec,wind_speed,rel_hum,radiation,state,sc,rc);
        TS_ASSERT_DELTA(rc.avg_discharge.value(0)*dt*1000.0/cell_area,prec.value(0),0.001);
        state.us.lwc=1.0;
        state.us.acc_melt=300.0;
        ...
        //state.us.sdc_melt_mean=10.0;
        state.us.acc_melt=5.0;//simulate early autumn, melt out everything
        state.us.temp_swe = 3.0;//
        state.us.lwc = 10.0;//
